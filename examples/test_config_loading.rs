...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  - Polling interval: {}ms", system_config.system.polling_interval_ms);
    println!("  - Log level: {:?}", system_config.system.log_level);
    println!("  - Threading enabled: {}", system_config.system.enable_threading);
    ...
    for (device_name, device_config) in &system_config.devices {
        ...
        if !device_config.max7219_displays.is_empty() {
            ...
            for (display_name, display_config) in &device_config.max7219_displays {
                println!("    - {}: CS pin {}, intensity {}, {} mode", 
                        display_name, 
                        display_config.cs_pin, 
                        display_config.intensity,
                        if display_config.raw_segments { "raw segments" } else { "numeric" });
        ...
        if !device_config.max7219_multi.is_empty() {
            println!("  📺 MAX7219 Multi-Displays: {}", device_config.max7219_multi.len());
            for (multi_name, multi_config) in &device_config.max7219_multi {
                println!("    - {}: {} displays, {} mode", 
                        multi_name, 
                        multi_config.displays.len(),
                        if multi_config.synchronized { "synchronized" } else { "independent" });
                
                for display in &multi_config.displays {
                    println!("      * {}: CS pin {}, intensity {}", 
    ...
    for (device_name, device_config) in &system_config.devices {
        ...
        println!("  Individual displays: {}", status.individual_displays.len());
