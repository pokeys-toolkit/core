...
fn main() -> Result<()> {
    ...
    println!(
        "\n🧪 Testing I2C operations with device at 0x{:02X}",
        test_address
    );
            ...
            "--serial" | "-s" => {
                if i + 1 < args.len() {
                    ...
                    return Err(PoKeysError::Parameter(
                        "--serial requires a value".to_string(),
                    ));
                }
            }
            "--address" | "-a" => {
                if i + 1 < args.len() {
                    ...
                    return Err(PoKeysError::Parameter(
                        "--address requires a value".to_string(),
                    ));
                }
            }
            "--help" | "-h" => {
    ...
    match device.i2c_write(address, &[time_addr]) {
        Ok(I2cStatus::Ok) => {
            match device.i2c_read(address, 7) {
                ...
                Ok((status, data)) => {
                    println!("⚠️  RTC read status: {:?}, data: {:02X?}", status, data)
                }
    ...
    match device.i2c_write(address, &[temp_reg]) {
        Ok(I2cStatus::Ok) => {
            match device.i2c_read(address, 2) {
                ...
                Ok((status, data)) => println!(
                    "⚠️  Temperature sensor status: {:?}, data: {:02X?}",
                    status, data
                ),
    ...
    match device.i2c_write(address, &test_data) {
        Ok(I2cStatus::Ok) => {
            ...
            match device.i2c_read(address, 3) {
                ...
                Ok((status, data)) => {
                    println!("⚠️  Read status: {:?}, data: {:02X?}", status, data)
                }
