...
fn main() -> Result<()> {
    ...
    let mut device = match connect_to_device_with_serial(device_serial, true, timeout_ms) {
        ...
        Err(e) => {
            ...
            println!("💡 Make sure:");
            println!("   - Device {} is powered on and connected to network", device_serial);
...
fn test_basic_operations(device: &mut PoKeysDevice, address: u8) -> Result<()> {
    println!("\n🧪 Testing basic operations with device at 0x{:02X}", address);
    ...
    match device.i2c_read(address, 3) {
        ...
        Ok((status, data)) => {
            println!("⚠️  Read status: {:?}, data: {:02X?} (device may not be present)", status, data);
        }
...
fn test_eeprom_operations(device: &mut PoKeysDevice, address: u8) -> Result<()> {
    ...
    println!("📝 Writing '{}' to EEPROM address 0x{:04X}", 
             String::from_utf8_lossy(test_data), register);
    ...
    match device.i2c_write(address, &write_data) {
                    ...
                    match device.i2c_read(address, test_data.len() as u8) {
                        Ok((I2cStatus::Ok, data)) => {
                            ...
                            if data == test_data {
                                ...
                                println!("⚠️  Data mismatch - wrote: {:02X?}, read: {:02X?}", test_data, data);
        ...
        Ok(status) => println!("⚠️  EEPROM write status: {:?} (device may not be present)", status),
