...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let device_config = config.devices.get("demo_device")
        .ok_or("Device 'demo_device' not found in configuration")?;
    
    println!("   Device: {}", device_config.name);
    println!("   Description: {}", device_config.description.as_ref().unwrap_or(&"None".to_string()));
    ...
    let mut device = match connect_to_device_with_serial(target_serial, true, 3000) {
        ...
        Err(_) => {
            println!("⚠️  Target device {} not found, trying any available device...", target_serial);
...
fn test_configured_device(
    ...
    while start_time.elapsed() < Duration::from_secs(5) {
        ...
        if counter % 10 == 0 {
            ...
            println!("   Status: Start={}, Stop={} | Time: {}s remaining", 
...
fn cleanup_device(
    ...
    for (pin_num, pin_config) in &config.pins {
        if pin_config.enabled && matches!(pin_config.function, pokeys_config::PinFunction::DigitalOutput) {
