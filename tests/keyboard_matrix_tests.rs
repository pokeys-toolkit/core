...
fn test_get_key_state_index_calculation() {
    ...
    // Test the key index calculation: row * width + col
    // For a 3x3 matrix:
    // (0,0) = 0, (0,1) = 1, (0,2) = 2
    // (1,0) = 3, (1,1) = 4, (1,2) = 5
    // (2,0) = 6, (2,1) = 7, (2,2) = 8
...
fn test_realistic_keyboard_scenarios() {
    ...
    kb.key_values[0] = 1;  // (0,0) = 0*4 + 0 = 0
    kb.key_values[5] = 1;  // (1,1) = 1*4 + 1 = 5
    kb.key_values[10] = 1; // (2,2) = 2*4 + 2 = 10
    kb.key_values[15] = 1; // (3,3) = 3*4 + 3 = 15
...
fn test_full_keyboard_press() {
    ...
    assert!(kb.get_key_state(1, 0)); // index 2
    assert!(kb.get_key_state(1, 1)); // index 3
}
