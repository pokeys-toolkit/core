...
impl AxisConfigBuilder {
    ...
    pub fn build(self, device: &mut PoKeysDevice) -> Result<()> {
        ...
        // Convert from pulses/s^2 to timeslot units (divide by 500 for acceleration)
        device.pulse_engine_v2.max_acceleration[self.axis] = (self.max_acceleration as f32) / 500.0;
        device.pulse_engine_v2.max_deceleration[self.axis] = (self.max_deceleration as f32) / 500.0;
