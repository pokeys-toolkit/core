...
impl StepResolution {
    pub fn to_u8(self) -> u8 {
        match self {
            StepResolution::FullStep => step_setting::FULL_STEP,
            StepResolution::HalfStep => step_setting::HALF_STEP,
            StepResolution::QuarterStep => step_setting::QUARTER_STEP,
            StepResolution::EighthStep => step_setting::EIGHTH_STEP,
            StepResolution::SixteenthStep => step_setting::SIXTEENTH_STEP,
        }
    }

    pub fn multiplier(self) -> f32 {
        match self {
            StepResolution::FullStep => 1.0,
            StepResolution::HalfStep => 0.5,
            StepResolution::QuarterStep => 0.25,
            StepResolution::EighthStep => 0.125,
            StepResolution::SixteenthStep => 0.0625,
        }
    }
...
impl PulseEngineV2 {
    ...
    pub fn get_axis_state(&self, axis: usize) -> PulseEngineAxisState {
        ...
        match self.axes_state[axis] {
            ...
            3 => PulseEngineAxisState::Accelerating,
            4 => PulseEngineAxisState::Decelerating,
...
impl PoKeysDevice {
    ...
    pub fn set_axis_configuration(&mut self, axis: usize) -> Result<()> {
        ...
        let mut axis_options = 0x01; // aoENABLED
        axis_options |= 1 << 2; // aoINTERNAL_PLANNER - enables acceleration/deceleration control
        axis_options |= 1 << 3; // aoPOSITION_MODE - internal position control mode
        ...
        // Bytes 17-20: Maximum speed (32-bit float in timeslot units)
        // Convert from pulses/second to timeslot units for 25kHz internal pulse frequency
        // timeslot_units = pulses_per_second / 25000
        let speed_timeslots = self.pulse_engine_v2.max_speed[axis] / 25000.0;
        let speed_bytes = speed_timeslots.to_le_bytes();
        request[8..12].copy_from_slice(&speed_bytes);

        // Bytes 21-24: Maximum acceleration (32-bit float in timeslot units)
        let accel_timeslots = self.pulse_engine_v2.max_acceleration[axis] / 25000.0;
        let accel_bytes = accel_timeslots.to_le_bytes();
        request[12..16].copy_from_slice(&accel_bytes);

        // Bytes 25-28: Maximum deceleration (32-bit float in timeslot units)
        let decel_timeslots = self.pulse_engine_v2.max_deceleration[axis] / 25000.0;
        let decel_bytes = decel_timeslots.to_le_bytes();
    ...
    pub fn get_axis_configuration(&mut self, axis: usize) -> Result<()> {
        ...
        if response.len() >= 40 {
            // Debug: Read back the values we just set
            ...
            let readback_speed = f32::from_le_bytes(speed_bytes);
            let readback_accel = f32::from_le_bytes(accel_bytes);
            let readback_decel = f32::from_le_bytes(decel_bytes);

            println!(
                "DEBUG 0x85/0x10 READBACK: speed={:.6}, accel={:.6}, decel={:.6}",
                readback_speed, readback_accel, readback_decel
            );
            println!(
                "DEBUG ORIGINAL VALUES: speed={:.6}, accel={:.6}, decel={:.6}",
                self.pulse_engine_v2.max_speed[axis],
                self.pulse_engine_v2.max_acceleration[axis],
                self.pulse_engine_v2.max_deceleration[axis]
            );
        }
    ...
    pub fn move_axis_to_position(&mut self, axis: usize, position: i32, _speed: f32) -> Result<()> {
        ...
        self.get_pulse_engine_status()?;

        // Set the reference position for the axis
        self.pulse_engine_v2.reference_position_speed[axis] = position;

        // Execute the move using 0x85/0x20 command
        // Now that axis is in position control mode, this should move to exact position
        let mut request = vec![0u8; 56]; // Data payload bytes 9-64

        // Send target position for moving axis, current positions for others
        for i in 0..8 {
            let pos = if i == axis {
                position // Target position for moving axis
            } else {
                self.pulse_engine_v2.current_position[i] // Current position for other axes
            };
    ...
    pub fn set_motor_drivers_configuration(&mut self) -> Result<()> {
        ...
        // Set motor driver settings for each axis (following official PoKeysLib)
        // device->request[8 + i*2] = pe->InternalDriverStepConfig[i];
        // device->request[9 + i*2] = pe->InternalDriverCurrentConfig[i];
        // But our request starts at protocol byte 9, so we need offset -1:
        // Axis 0: protocol bytes 8,9 -> request[-1,0] (invalid)
        // Axis 1: protocol bytes 10,11 -> request[1,2]
        // Axis 2: protocol bytes 12,13 -> request[3,4]
        // Axis 3: protocol bytes 14,15 -> request[5,6]

        for axis in 0..4 {
            let protocol_offset = 9 + axis * 2; // Changed from 8 to 9
            if protocol_offset >= 9 {
                // Only if within our request range (starts at byte 9)
                let request_offset = protocol_offset - 9; // Convert to request array index
                if request_offset + 1 < request.len() {
                    request[request_offset] = self.pulse_engine_v2.motor_step_setting[axis];
                    request[request_offset + 1] = self.pulse_engine_v2.motor_current_setting[axis];
                }
            }
        }

        println!(
            "DEBUG 0x85/0x19 REQUEST: Axis 2 -> protocol bytes 13,14 -> request[4,5] = step={}, current={}",
            request[4], request[5]
        );
        ...
        println!(
            "DEBUG 0x85/0x19 RESPONSE: protocol bytes 13,14 = step={}, current={}",
            response.get(13).unwrap_or(&255),
            response.get(14).unwrap_or(&255)
        );
    ...
    pub fn get_motor_drivers_configuration(&mut self) -> Result<()> {
        ...
        for axis in 0..4 {
            ...
            if byte_offset + 1 < response.len() {
                ...
                // Device returns 0-based values which match our 0x85/0x19 command format
